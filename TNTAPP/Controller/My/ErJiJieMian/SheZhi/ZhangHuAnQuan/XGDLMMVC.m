...
@interface XGDLMMVC ()<UITextFieldDelegate>
{
    UITextField * _yuanmimatext;
    UITextField * _xinmimatext1;
    UITextField * _xinmimatext2;
...
-(void)zdyUI
{
    ...
    NSArray * array = @[@" ",@"请输入当前密码",@"请输入6-18位新密码",@"请再次输入新密码"];
    ...
    UIButton * wjmmbtn = [[UIButton alloc] initWithFrame:CGRectMake(APPwidth-120, _tjbtn.frame.origin.y+_tjbtn.frame.size.height+10,105, 30)];
    [wjmmbtn setTitle:@"忘记密码?" forState:0];
    [wjmmbtn setTitleColor:[UIColor whiteColor] forState:0];
    [wjmmbtn addTarget:self action:@selector(wjmm) forControlEvents:UIControlEventTouchUpInside];
    wjmmbtn.titleLabel.font = [UIFont systemFontOfSize:16];
