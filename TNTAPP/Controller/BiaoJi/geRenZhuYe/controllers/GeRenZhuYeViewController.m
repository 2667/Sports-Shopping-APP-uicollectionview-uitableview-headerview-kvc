...
#define CATEGORY  @[@"任务",@"标记",@"圈子",@"运动",@"相册"]
...
@interface GeRenZhuYeViewController ()<UIScrollViewDelegate,JQHeaderViewDelegate,geRenHeaderViewDelegate,XiangCeViewControllerDelegate>
...
//
@property(nonatomic,assign)BOOL IfXiangCeBtnClicked;


@end


@implementation GeRenZhuYeViewController
...
//        }
    ...
    if (index>=0&& index<4) {
        ...
        if (_IfXiangCeBtnClicked==NO) {
            
            
            self.lastTableViewOffsetY = tableViewoffsetY;
        }else{
        
            _IfXiangCeBtnClicked=NO;
            tableViewoffsetY = self.lastTableViewOffsetY;
//            tableViewoffsetY = 0;
        }
...
//                         animations:^{
//                             
//                             self.cycleScrollView.frame = CGRectMake(0, 0, SCREEN_WIDTH, 244);
//                             self.segmentScrollView.frame = CGRectMake(0, 244, SCREEN_WIDTH, 40);
//                             
//                         }
...
//        UICollectionView *tableView = self.CurrentCollectionView;
//        CGFloat tableViewoffsetY = tableView.contentOffset.y;
//        
//        self.lastTableViewOffsetY = tableViewoffsetY;
...
//             self.currentSelectedItemImageView.frame = CGRectMake(0, self.segmentScrollView.frame.size.height - 2,currentButton.frame.size.width*1.3, 7);

              self.currentSelectedItemImageView.frame = CGRectMake(0, self.segmentScrollView.frame.size.height - 10,currentButton.frame.size.width*1.3, 10);
            ...
            //标题按钮底部下划线的frame
//            self.currentSelectedItemImageView.frame = CGRectMake(currentButton.centerX-currentButton.frame.size.width*1.3/2, self.segmentScrollView.frame.size.height-2, currentButton.frame.size.width*1.3, 7);
            
            //标题按钮底部下划线的frame
            self.currentSelectedItemImageView.frame = CGRectMake(currentButton.centerX-currentButton.frame.size.width*1.3/2, self.segmentScrollView.frame.size.height-10, currentButton.frame.size.width*1.3, 10);
...
//                self.lastTableViewOffsetY = tableViewoffsetY;
        self.lastTableViewOffsetY = 0;
        _IfXiangCeBtnClicked= YES;
        NSLog(@"tableViewoffsetYyyy内容是%f",tableViewoffsetY);
...
//            self.currentSelectedItemImageView.frame = CGRectMake(currentButton.centerX-currentButton.frame.size.width*1.3/2, self.segmentScrollView.frame.size.height-10, currentButton.frame.size.width*1.3, 10);
//            NSLog(@"currentButton.centerX-currentButton.frame.size.width*1.3/2内容是%f",currentButton.centerX-currentButton.frame.size.width*1.3/2);
...
//            self.currentSelectedItemImageView.frame = CGRectMake(CGRectGetMinX(currentButton.frame), self.segmentScrollView.frame.size.height-2, currentButton.frame.size.width*1.3, 2);
...
//            JSDTableViewController *jsdTableViewController = [[JSDTableViewController alloc] init];
    ...
    RenWuZaiBiaoJiVC *jsdTableViewController = [[RenWuZaiBiaoJiVC alloc]init];
...
//                    [self addChildViewController:jsdTableViewController];
                ...
                case 1:
                {
        BiaoJiZaiGeRenTVC *BiaoJiZaiGeRenvc = [[BiaoJiZaiGeRenTVC alloc]init];
                ...
                case 2:
                {
                   QuanziTableViewController * jsdTableViewController = [[QuanziTableViewController alloc]init];
...
//                     [jsdTableViewController.tableHeaderView addObserver:self forKeyPath:@"contentOffset" options:options context:nil];
                ...
                case 3:
                {
   yunDongViewController *jsdTableViewController = [[yunDongViewController alloc]init];
...
//                    jsdTableViewController = [[BiaoJiZaiGeRenTVC alloc]init];
    ...
    jsdTableViewController.view.frame = CGRectMake(SCREEN_WIDTH * i, 0, SCREEN_WIDTH, SCREEN_HEIGHT);
                ...
                case 4:
                {
                XiangCeViewController   *jsdTableViewController = [[XiangCeViewController alloc]init];
                    jsdTableViewController.delegate =self;
//    _collectionView = [[UICollectionView alloc]initWithFrame:[UIScreen mainScreen].bounds collectionViewLayout:self.layout];
                    
//                    tbCollectionViewController * jsdTableViewController = [[tbCollectionViewController alloc]initWithCollectionViewLayout:[[UICollectionViewFlowLayout alloc]init]];
                    ...
                    [self.tableViews addObject:jsdTableViewController.collectionView];
                    
                    NSKeyValueObservingOptions options = NSKeyValueObservingOptionNew | NSKeyValueObservingOptionOld;
                    [jsdTableViewController.collectionView addObserver:self forKeyPath:@"contentOffset" options:options context:nil];
...
//                _currentSelectedItemImageView.frame = CGRectMake(0, self.segmentScrollView.frame.size.height - 10, btn.frame.size.width*1.3, 10);
                
                self.currentSelectedItemImageView.frame = CGRectMake(btn.centerX-btn.frame.size.width*1.3/2, self.segmentScrollView.frame.size.height-10, btn.frame.size.width*1.3, 10);
//                _currentSelectedItemImageView.frame = CGRectMake(0, self.segmentScrollView.frame.size.height - 2, btn.frame.size.width*1.3, 2);
            }
        }
...
//    [_headerView.guanZhuBtn.button setBackgroundImage:[UIImage imageNamed:@"biaoji_gerenzhuye_guanzhu"] forState:UIControlStateNormal];
    ...
    //进店看看
    _headerView.JinDianKanKanView.userInteractionEnabled = YES;
    UITapGestureRecognizer* singleTapJin = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(JinDianKanKanBtnClicked:)];
    [_headerView.JinDianKanKanView.view  addGestureRecognizer:singleTapJin];
...
-(void)JinDianKanKanBtnClicked:(shangPuHeGeRenZhuYeView*)shangPuHeGeRenZhuYeView{

    ShangJiaZhuYeViewController *guanZuVC = [[ShangJiaZhuYeViewController alloc]init];
